...
## Not Yet Done
- **Contact Us page** (name + message form → email to paul@vaultguards.co). Needs a decision on which transactional email API to use (Resend/SendGrid/Mailgun) since Cloudflare Workers can't send SMTP directly — API key would be stored as a Cloudflare secret. Nav/Footer "Contact" links still point to placeholders until this exists.
